...
type
  ...
  published
    ...
    property BackupFile: TStrings read FBackupFile write FBackupFile;
  ...
  published
    { a name=value pair of filename and length }
    property DatabaseName: TStrings read FDatabaseName write FDatabaseName;
    property BackupFile: TStrings read FBackupFile write FBackupFile;
...
begin
  ...
  if call(isc_service_attach(StatusVector, Length(ConnectString),
                         ...
                         FSPBLength, FSPB), False) > 0 then begin
    FHandle := nil;
    IBDataBaseError;
  end;
...
begin
  ...
  try
    if call(isc_service_query(StatusVector, @FHandle, nil, 0, nil,
                           ...
                           FBufferSize, FOutputBuffer), False) > 0 then begin
      FHandle := nil;
      IBDataBaseError;
    end;
  finally
    IBAlloc(FQuerySPB, 0, 0);
    FQuerySPB := nil;
    FQuerySPBLength := 0;
    FQueryParams := '';
  end;
...
begin
  ...
  for i := 0 to Params.Count - 1 do begin
    ...
    if (pos_of_str = 1) or (pos_of_str = Length(SPBPrefix) + 1) then begin
      result := i;
      break;
    end;
  end;
  ...
  for i := 0 to sl.Count - 1 do begin
   { Get the parameter's name and value from the list,
    ...
    for j := 1 to isc_spb_last_spb_constant do
      if (param_name = SPBConstantNames[j]) then begin
        SPBVal := j;
        SPBServerVal := SPBConstantValues[j];
        break;
      end;
    case SPBVal of
      isc_spb_user_name, isc_spb_password: begin
        SPB := SPB +
               Char(SPBServerVal) +
...
begin
  Inc(RunLen);
  with FConfigParams.ConfigFileData do begin
...
begin
  if (Database in Options) then FetchDatabaseInfo;
  if (License in Options) then FetchLicenseInfo;
  if (LicenseMask in Options) then FetchLicenseMaskInfo;
  if (ConfigParameters in Options) then FetchConfigParams;
  if (Version in Options) then FetchVersionInfo;
end;
...
begin
  ...
  RunLen := 0;
  While (not (Integer(OutputBuffer[RunLen]) = isc_info_end)) do begin
    ...
    case Integer(OutputBuffer[RunLen]) of
      isc_info_svc_get_config: begin
        ...
        Inc (RunLen);
        while (not (Integer(OutputBuffer[RunLen]) = isc_info_flag_end)) do begin
  ...
  While done < 2 do begin
    ...
    isc_info_svc_get_license:
      begin
        while (OutputBuffer[RunLen] <> Char(isc_info_flag_end)) do
        begin
          if (i >= Length(FLicenseInfo.key)) then begin
        ...
        Inc(RunLen);
        if (Length(FLicenseInfo.key) > i) then begin
...
begin
  ...
  try
    if call(isc_service_start(StatusVector, @FHandle, nil,
                           FStartSPBLength, FStartSPB), False) > 0 then begin
      FHandle := nil;
      IBDataBaseError;
    end;
  finally
    IBAlloc(FStartSPB, 0, 0);
    FStartSPB := nil;
    FStartSPBLength := 0;
    FStartParams := '';
  end;
...
begin
  if FDatabaseName = '' then
    IBError(ibxeStartParamsError, [nil]);
  param := 0;
  if (DataPages in Options) then param := param or isc_spb_sts_data_pages;
  if (DbLog in Options) then param := param or isc_spb_sts_db_log;
  if (HeaderPages in Options) then param := param or isc_spb_sts_hdr_pages;
  if (IndexPages in Options) then param := param or isc_spb_sts_idx_pages;
  if (SystemRelations in Options) then param := param or isc_spb_sts_sys_relations;
...
begin
  if FDatabaseName = '' then
    IBError(ibxeStartParamsError, [nil]);
  param := 0;
  if (IgnoreChecksums in Options) then param := param or isc_spb_bkp_ignore_checksums;
  if (IgnoreLimbo in Options) then param := param or isc_spb_bkp_ignore_limbo;
  if (MetadataOnly in Options) then param := param or isc_spb_bkp_metadata_only;
  if (NoGarbageCollection in Options) then param := param or isc_spb_bkp_no_garbage_collect;
  if (OldMetadataDesc in Options) then param := param or isc_spb_bkp_old_descriptions;
  if (NonTransportable in Options) then param := param or isc_spb_bkp_non_transportable;
  if (ConvertExtTables in Options) then param := param or isc_spb_bkp_convert;
  ...
  for i := 0 to FBackupFile.Count - 1 do begin
    ...
    if (Pos('=', FBackupFile[i]) <> 0) then begin {mbcs ok}
...
begin
  param := 0;
  if (DeactivateIndexes in Options) then param := param or isc_spb_res_deactivate_idx;
  if (NoShadow in Options) then param := param or isc_spb_res_no_shadow;
  if (NoValidityCheck in Options) then param := param or isc_spb_res_no_validity;
  if (OneRelationAtATime in Options) then param := param or isc_spb_res_one_at_a_time;
  if (Replace in Options) then param := param or isc_spb_res_replace;
  if (CreateNewDB in Options) then param := param or isc_spb_res_create;
  if (UseAllSpace in Options) then param := param or isc_spb_res_use_all_space;
  ...
  for i := 0 to FDatabaseName.Count - 1 do begin
    if (Trim(FDatabaseName[i]) = '') then continue;
    if (Pos('=', FDatabaseName[i]) <> 0) then begin {mbcs ok}
...
begin
  ...
  i := 0;
  FLimboTransactionInfo := nil;
  while (OutputBuffer[RunLen] <> Char(isc_info_end)) do begin
    if (i >= Length(FLimboTransactionInfo)) then
      SetLength(FLimboTransactionInfo, i + 10);
    with FLimboTransactionInfo[i] do
    begin
      if (OutputBuffer[RunLen] = Char(isc_spb_single_tra_id)) then begin
        Inc(RunLen);
        MultiDatabase := False;
        ...
        if (Value = Char(isc_spb_tra_state_limbo)) then
          State := LimboState
        else if (Value = Char(isc_spb_tra_state_commit)) then
          State := CommitState
        else if (Value = Char(isc_spb_tra_state_rollback)) then
          State := RollbackState
        else
          State := UnknownState;
        ...
        Value := OutputBuffer[RunLen];
        Inc(RunLen);
        if (Value = Char(isc_spb_tra_advise_commit)) then begin
          Advise := CommitAdvise;
          Action:= CommitAction;
        end
        else if (Value = Char(isc_spb_tra_advise_rollback)) then begin
...
begin
  ...
  ServiceStartAddParam (FDatabaseName, SPBConstantValues[isc_spb_dbname]);
  if (FGlobalAction = NoGlobalAction) then begin
    i := 0;
    while (FLimboTransactionInfo[i].ID <> 0) do begin
...
begin
  ...
  ServiceStartParams  := Char(isc_action_svc_repair);
  ServiceStartAddParam (FDatabaseName, SPBConstantValues[isc_spb_dbname]);
  if param > 0 then ServiceStartAddParam (param, SPBConstantValues[isc_spb_options]);
  param := 0;
  if (LimboTransactions in Options) then param := param or isc_spb_rpr_list_limbo_trans;
  if (CheckDB in Options) then param := param or isc_spb_rpr_check_db;
  if (IgnoreChecksum in Options) then param := param or isc_spb_rpr_ignore_checksum;
...
begin
  ...
  if (OutputBuffer[3 + Length] = Char(isc_info_truncated)) then
    FEof := False
  else if (OutputBuffer[3 + Length] = Char(isc_info_end)) then
    FEof := True
  else
    IBError(ibxeOutputParsingError, [nil]);
